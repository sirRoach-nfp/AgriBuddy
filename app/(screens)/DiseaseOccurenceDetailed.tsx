...
const DiseaseOccurrencesDetailed = () => {
  ...
  const displayData = (pestLogs: PestLog[], selectedPests: string[]) => {
    ...
    const filteredLogs = pestLogs.filter((log: any) => selectedPests.includes(log.Diseasename));
    console.log("Filtered Log:", filteredLogs);

    // Group data by pest name and month
    const groupedData: Record<string, Record<string, number>> = {};
    filteredLogs.forEach((log) => {
        const { Diseasename, Date } = log;
        const month = getMonthShortName(Date);

        if (!groupedData[Diseasename]) {
            groupedData[Diseasename] = {};
        }
        groupedData[Diseasename][month] = (groupedData[Diseasename][month] || 0) + 1;
    });
 ...
 const displayDataTempVOccu = (pestLogs:PestLog[],selectedPests:string[])=>{
    ...
    const pestTypes = [...new Set(filteredLogs.map(item=>item.Diseasename))];
    ...
    filteredLogs.forEach(({ Diseasename, Temp }) => {
        const tempIndex = uniqueTemps.indexOf(Temp);
        if (tempIndex !== -1) {
          pestCounts[Diseasename][tempIndex]++;
        }
      });
 ...
 const displayPieChartData = (pestLogs:PestLog[],selectedCrop:string[])=>{
    ...
    const pestCounts = pestLogs.reduce((acc, log) => {
        acc[log.Diseasename] = (acc[log.Diseasename] || 0) + 1;
        return acc;
  ...
  useEffect(()=> {
    ...
    const fetchPestLogRecord = async(plotId:String)=>{
            ...
            console.log("<---------- fetchPestLogRecord START ---------->")
            ...
            try{
                ...
                if(docSnap.exists()){
                    ...
                    const plotPestLog = existingLogs[logIndex].PlotDiseaseLog;

                    
                    console.log( "Filtered Plot Pest log entry", existingLogs[logIndex].PlotDiseaseLog)

                    const pestNames = [...new Set(plotPestLog.map((log: any) => log.Diseasename))];
                    ...
                    console.log("Plot pest log before storing to state :",existingLogs[logIndex].PlotPestLog)
                    setPestLogs(existingLogs[logIndex].PlotDiseaseLog)
            ...
            console.log("<---------- fetchPestLogRecord END ---------->")
  ...
  useEffect(()=> {
    ...
    const formatPestLogToCount = (PestLog:any) => {
        ...
        console.log("<---------- Format Pest Log To Count START---------->")
        ...
        PestLog.forEach((log:any)=> {
            ...
            const pestName = log.Diseasename;
            if(pestName){
        ...
        console.log("<---------- Format Pest Log To Count END ---------->")
        ...
        <View style={styles.headerContainer}>
            ...
            <ScrollView style={styles.scrollContainer} contentContainerStyle={{alignItems:'center'}}>
                ...
                <View style={[styleComparisonChart.wrapper,{display:'none'}]}>
                    ...
                    <View style={styleComparisonChart.headerWrapper}>
                        <Text style={styleComparisonChart.headerText}>
                            Disease Occurrence vs Temperature Trend
                        </Text>
                    </View >
                    ...
                    <View style={[styleComparisonChart.legendWrapper,{display:'none'}]}>
...
export default DiseaseOccurrencesDetailed
